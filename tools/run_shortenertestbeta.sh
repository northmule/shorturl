...
shortenertestbeta -test.v -test.run=^TestIteration10$ -source-path=$SHORTURL_PROJECT_PATH -binary-path=$SHORTURL_PROJECT_PATH/cmd/shortener/shortener -source-path=$SHORTURL_PROJECT_PATH -database-dsn='postgres://postgres:123@localhost:5456/shorturl?sslmode=disable'